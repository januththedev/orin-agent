...
export const BACKEND_RESTARTING =
  'Orin stopped unexpectedly — restarting and reopening your chat (the reply in progress was lost).'

export const BACKEND_RESTARTING_ACTIVITY = 'Orin stopped unexpectedly · restarting…'

// Attached (dashboard / embedded) mode: only the socket dropped; Orin and any
// reply in progress are still alive on the backend and come back on reconnect.
export const CONNECTION_LOST = 'Connection to Orin lost — reconnecting and reopening your chat…'
...
export const backendGaveUp = (code: null | number, lastLine?: string): string => {
  ...
  return [
    `Orin stopped${exit} and could not be restarted. Your chat is saved.`,
    detail,
    'Orin keeps trying to reconnect in the background and reopens this chat when it succeeds; if it does not, type /resume.',
...
export const BACKEND_GAVE_UP_ACTIVITY = 'Orin stopped · /logs for details'
...
export const stderrProblemActivity = (line: string): string => {
  ...
  return `Something went wrong inside Orin${what} · /logs for details`
}
...
const RPC_ERROR_ROWS: RpcErrorRow[] = [
  ...
  [
    ...
    () =>
      'Orin is not connected right now, so that was not sent. It reconnects automatically; wait a moment and try again, or type /logs if this persists.'
  ],
  [
    (_code, text) => TIMED_OUT_RE.exec(text),
    m =>
      `Orin did not answer within ${m?.[1] ?? '?'}s. Try again; if it keeps happening, type /logs and report the last lines.`
  ]
]
...
const TURN_LAYER_COPY: Record<string, [string, string]> = {
  ...
  billing: ['The model provider reports no credit left', 'Top up the account or switch with /model.'],
  disk: ['The disk is full, so Orin could not save the turn', 'Free some space, then /retry.'],
  endpoint: ['Your custom model endpoint did not answer', 'Check the endpoint is running, then /retry.'],
  gateway: ['Orin hit an internal error while running this turn', 'Send /retry; type /logs for the trace.'],
