...
Orin Agent package derivation, so only that build may create an artifact.
...
_BLOCK_MESSAGE = (
    "Building wheels or sdists for orin-agent is not supported.\n"
    "Orin Agent is distributed from source, Docker image, or Nix.\n"
    "See: https://github.com/januththedev/orin-agent\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Orin Agent Nix derivation sets HERMES_NIX_BUILD=1. If it does, file a bug."
)
