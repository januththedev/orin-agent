...
export const API_UNREACHABLE_MESSAGE =
  "Orin Agent dashboard cannot reach the Orin Agent service. Is `orin dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The Orin Agent service hit an internal error.",
  502: "The dashboard proxy could not reach the Orin Agent service.",
  503: "The Orin Agent service is not ready yet. Try again in a moment.",
  504: "The Orin Agent service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The Orin Agent service answered with an unexpected error (${status}).`;
}
