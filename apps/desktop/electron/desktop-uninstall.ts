 ...
 *
 *   macOS:   …/Orin.app/Contents/MacOS/Orin  → …/Orin.app
 *   Windows: …\Orin\Orin.exe                 → …\Orin  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Orin.app/Contents/MacOS/Orin → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Orin.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Orin.exe directly in the install dir.
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Orin install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
