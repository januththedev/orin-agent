...
test('exit line carries the buffered tail next to the exit code, preferring the signal', () => {
  ...
  assert.equal(
    formatBackendExitLine('Ignoring stale Orin backend exit', 1, null, tail),
    'Ignoring stale Orin backend exit (1)\nRecent backend output:\nTraceback (most recent call last):'
  )
  assert.equal(
    formatBackendExitLine('Orin backend exited', null, 'SIGTERM', tail),
    'Orin backend exited (SIGTERM)\nRecent backend output:\nTraceback (most recent call last):'
  )
})

test('exit line stays byte-identical to the legacy shape when the tail is empty or missing', () => {
  assert.equal(
    formatBackendExitLine('Orin backend exited', 0, null, createBackendOutputTail(64)),
    'Orin backend exited (0)'
  )
  assert.equal(formatBackendExitLine('Orin backend exited', 1, null, null), 'Orin backend exited (1)')
})
