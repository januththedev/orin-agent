...
const BOOTSTRAP_FAILURE_REMEDY =
  'Common causes: no internet connection, antivirus blocking the installer, or another copy of Orin running. ' +
  'Close other Orin windows and choose Reload and retry; if it fails again, open the logs and send them to support.'
...
export function describeBootstrapFailure(failedStage: string | null | undefined, rawError: unknown): string {
  ...
  const lead = label
    ? `Setting up Orin stopped during the '${label}' step.`
    : 'Setting up Orin stopped before it could finish.'
...
/**
 * Error.message for an installed Orin with a piece missing (source tree,
...
export function missingInstallPartMessage(whatIsMissing: string): string {
  return (
    "Part of Orin' installation is missing (it may have been deleted or quarantined by antivirus). " +
