 ...
 *     `X-Orin-Session-Token` header; WS uses `?token=`.
...
/** Which Orin profile the remote SSH dashboard should actually run as.
...
// Orin Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  // name used by the remote Orin installation. Preserve an explicit mapping
  // when it is a valid Orin profile identifier; otherwise fall back to the
