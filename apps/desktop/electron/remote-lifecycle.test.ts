...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Orin', async () => {
...
test('buildSpawnCommand raises the SSH child file limit before execing Orin', () => {
  const cmd = buildSpawnCommand('/x/hermes', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  const versionSsh = fakeSsh([
    [
      ...
      (cmd: string) => {
        ...
        return 'Orin Agent v0.18.2 (abc123)\n'
      }
    ]
  ])
