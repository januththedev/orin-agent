...
test('error page names the failure and carries a Reload button', () => {
  ...
  assert.match(html, /Orin couldn.t start the desktop UI/)
...
test('error page reload button targets the real renderer URL when provided', () => {
  const html = buildRendererLoadErrorPage({
    ...
    reloadUrl: 'file:///C:/Orin%20Agent/dist/index.html'
  })
  ...
  assert.match(html, /location\.replace\("file:\/\/\/C:\/Orin%20Agent\/dist\/index\.html"\)/)
...
test('a custom title overrides the default heading', () => {
  ...
  assert.match(html, /Orin desktop UI was terminated/)
  assert.doesNotMatch(html, /Orin couldn.t start the desktop UI/)
})
