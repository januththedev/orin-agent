...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Orin.app/Contents/MacOS/Orin', 'darwin'),
    '/Applications/Orin.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Orin.app/Contents/MacOS/Orin', 'darwin'),
    '/Users/x/Applications/Orin.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Orin\\Orin.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Orin'
  )
  assert.equal(
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Orin.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Orin.app'), false)
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Orin',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Orin" >nul 2>&1/)
