...
if (IS_WINDOWS) {
  ...
  // expose Orin sessions/config to every packaged app on the machine.
...
// always-on timers. A completely idle, minimized Orin burned ~20% CPU
...
// active-profile.json records which Orin profile the desktop launches its
...
// Seed the native About panel with the live Orin version. This is refreshed
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Orin backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
let bootProgressState = {
  ...
  message: 'Waiting to start Orin backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Orin will start automatically when it completes…',
        12
      )
    },
    ...
    if (result && result.ok && result.manual) {
      ...
      void dialog
        .showMessageBox({
          type: 'error',
          title: 'Orin update',
          message: "Orin couldn't finish updating",
          detail:
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Orin to load the updated app…', 14)
...
async function findSystemPython() {
  ...
  // CPython but several Orin deps (notably pywinpty's Rust-built
...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message:
        "This copy of Orin can't update itself from inside the app. Download the latest version from the Orin website, " +
        `or reinstall Orin to enable in-app updates. Details: ${updateRoot} has no version-control metadata.`,
...
// Kill only Orin-OWNED venv daemons (the memory plugin's hindsight daemon:
...
function killHermesOwnedVenvDaemons(updateRoot) {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Orin-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Orin backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Orin backend PID ${child.pid}; ` +
  ...
  try {
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Orin backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  // Reap Orin-OWNED venv daemons the tree-kill above cannot reach: the
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Orin — this window will close and the updater will open. Don’t reopen Orin yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Orin install open ' +
    ...
    // Orin processes running from this venv.  The updater normally refuses
    ...
    if (scriptHandoff) {
      ...
      // abort with "Another Orin update is already running (PID <itself>)" —
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  // The updater invokes the venv's Orin launcher, which in turn requires the
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Orin — this window will close. Don’t reopen Orin yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Orin.app/Contents/...`
...
async function resolveHermesBackend(backendArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = await createPythonBackend(overrideRoot, `Orin source at ${overrideRoot}`, backendArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = await createPythonBackend(SOURCE_REPO_ROOT, `Orin source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  return {
    ...
    label: 'Orin Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Orin install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Orin recovery was handed off to Orin Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Orin install was cancelled.') as any
  ...
  // On Windows, preflight Git Bash. Orin' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Orin needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen Orin.'
    )
  }
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Orin backend URL protocol: ${parsed.protocol}`))
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Orin backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Orin-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Orin-Session-Token`` header — used by the auth-mode probe before
...
// signal so the chat composer doesn't stay stuck on "Starting Orin...".
...
//
// Hosted Orin gateways gate the dashboard behind an OAuth provider (e.g.
...
// single-shot boot() with no retry) surfaced as the "Orin couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Orin
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Orin Cloud agent…' : 'Sign in to Orin gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Orin backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Orin Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// so a single override flips every Orin surface to the same portal.
...
// Discover the hosted (Orin Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Orin Cloud. Open Settings → Gateway, choose Orin Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Orin Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Orin Cloud session has expired. Sign in to Orin Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Orin Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    remoteUrl,
    // The persisted Orin Cloud org (slug/id) for a cloud connection, or '' for
...
// `org` (optional) is the Orin Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Orin Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote Orin gateway is selected, but no session token is saved. ' +
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Orin
...
async function testDesktopConnectionConfig(input: any = {}) {
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Orin on the remote host before connecting with Desktop SSH.'
            }
          }
  ...
  // connect to Orin gateway". Mirror the renderer's connect here so the test
...
async function runPoolBackendStart(
  ...
  rememberLog(`Starting Orin backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Orin backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Orin backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(formatBackendExitLine(`Orin backend for profile "${profile}" exited`, code, signal, outputTail))
    releaseBackendChild(child)

    if (!ready) {
      rejectStart?.(
        new Error(
          `Orin backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Orin backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Orin backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
function startAttachedBackendMonitor(attached: AttachedBackend) {
  ...
  attachedBackendMonitor = setInterval(() => {
    void waitForHermes(attached.baseUrl, attached.token, undefined, 'token', {}).catch(() => {
      ...
      scheduleUnexpectedPrimaryRecovery({ error: 'The Orin backend this app attached to exited.', ready: true })
    })
...
function scheduleUnexpectedPrimaryRecovery({
  ...
  if (!claimed) {
    if (primaryExitRecovery.isCrashLooping()) {
      const message =
        'Orin backend keeps crashing right after it restarts; not restarting it again. Relaunch Orin Desktop.'
...
async function runHermesStart() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Orin Desktop is already running in another window.')
  }
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Orin backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Orin backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Orin backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Orin runtime', 28)
    ...
    if (setup.kind === 'attached') {
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Attached to the running Orin backend',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.spawn', `Starting Orin backend via ${backend.label}`, 84)
    rememberLog(`Starting Orin backend via ${backend.label}`)
    ...
    if (!processOwner) {
      ...
      throw new Error('Orin backend start was superseded by a newer connection attempt.')
    }
    ...
    hermesProcess.once('error', error => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Orin backend error: ${error.message}`)
        scheduleUnexpectedPrimaryRecovery({ error: error.message, ready: backendReady })
        rejectBackendStart?.(new Error('Orin backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Orin backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Orin backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(formatBackendExitLine('Ignoring stale Orin backend exit', code, signal, primaryOutputTail))
      ...
      rememberLog(formatBackendExitLine('Orin backend exited', code, signal, primaryOutputTail))
      ...
      if (!backendReady) {
        const message = `Orin backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    await advanceBootProgress('backend.port', 'Waiting for Orin backend to launch', 86)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Orin backend to become ready', 90)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Orin backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Orin backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Orin is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Orin app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Orin icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Orin in the normal
...
// window showing only the composer and its scrollback, so Orin can be driven
...
// left the user with NO Orin window after the second toggle (#88513).
...
function createWindow() {
  ...
  installWindowRendererLifecycle(mainWindow, {
    ...
    callbacks: {
      ...
      // reclaim, an external kill). Orin never does this itself and never reloads it
      ...
      onRendererTerminated: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          title: 'Orin desktop UI was terminated',
          errorDescription:
...
// "Starting Orin…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    if (!backend.command) {
      return { ok: false, error: 'Orin is not installed yet' }
    }
...
// --- Orin Cloud (cloud-auto-discovery Phase 3) ---
...
// Resolve the canonical Orin version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Orin version instead of the Electron app's own package.json version,
...
// Re-resolve the live Orin version and push it into the native About panel
...
// The About page's "Restart Orin" button (shown when bundleSwapPending):
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Orin agent venv at ${VENV_ROOT}.`
    }
  }
...
app.on('before-quit', event => {
  ...
  // quitting Orin never keeps another app's chord hostage.
