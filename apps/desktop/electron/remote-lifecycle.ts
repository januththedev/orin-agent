 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Orin dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Orin install on the remote (login-shell probe),
...
async function locateHermes(ssh, remoteHermesPath) {
  ...
  const err: any = new Error(
    'Hermes is not installed on the remote host (could not find a `hermes` executable). ' +
      ...
      '— or set the Orin path explicitly in the SSH connection settings.'
  )
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Orin Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
async function probeRemoteHermesHome(ssh) {
  try {
    ...
    const error: any = new Error('Could not resolve the remote Orin home.')
 ...
 * bytes; it never imports or executes code from the changing Orin checkout.
...
async function assertRemoteInstallUpdateClear(ssh, hermesHome) {
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Orin install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Orin update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Orin update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteHermesProfiles(ssh) {
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Orin profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
async function readRemoteInstallId(ssh) {
  ...
  try {
    ...
    const error: any = new Error('Could not read the remote Orin install id.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Orin home.')
...
// execing Orin. mutexPath is expandRemotePath() output — a complete shell
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, hermesPath))) {
    const err: any = new Error(
      'The remote Orin install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Orin on the remote host to continue using Desktop SSH mode.'
    )
...
async function connect(deps) {
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Orin Desktop build (${lock.reason}). ` +
