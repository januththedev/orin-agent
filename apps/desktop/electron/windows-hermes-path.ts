 ...
 *      and Orin launcher. Marker-only or partial runtimes must use --repair;
...
export async function resolveVenvHermesCommand(
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Orin at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Orin Python at ${python}`,
    command: python,
