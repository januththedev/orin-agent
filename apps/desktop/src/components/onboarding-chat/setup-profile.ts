 ...
 * only Orin and the title `Welcome to Orin`.
 *
...
export const SETUP_CHAT_TITLE = 'Welcome to Orin'
...
export function buildFirstTaskRunbook(
  ...
  return [
    `You are Orin. The user's welcome chat just opened this session so one task can have room to run: ${task.trim()}.`,
...
const pluginRunbook = (root: string) => [
  'THIS IS A PLUGIN JOB: the thing you are building is a piece of the Orin app itself, and it will appear in the window the user is looking at right now. That is the whole point — do not let it become a script in a folder.',
  ...
  'Never ask them to restart the app, never edit anything outside their plugin folder, and never touch the Orin install itself. If the plugin errors on load, the app toasts it and keeps running — read the error, fix the file, save again.'
]
...
export async function ensureSetupProfile(request: GatewayRequest): Promise<void> {
  try {
    await request('profiles.create', {
      description: 'Where Orin met you — walks your first run, then checks in as you find your feet.',
      name: SETUP_PROFILE,
      clone_from: 'default',
      share_auth: true,
      no_alias: true,
