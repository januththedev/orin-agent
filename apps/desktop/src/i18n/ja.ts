...
export const ja = defineLocale({
  sessionImport: {
    title: '別のアプリから続ける',
    subtitle: '会話をOrinに取り込み、続きを始めましょう。',
    action: 'セッションを取り込む',
    readingFrom: '読み込み元',
  ...
  boot: {
    ready: 'Orin Desktop の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'Orin の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      retryingRemoteBackend: 'リモート Orin バックエンドに再接続中…',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingHermesDesktop: 'Orin Desktop を起動中…'
    },
    errors: {
      backgroundExited: 'Orin バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に Orin バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Orin バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateHermes: 'Orin を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Orin Desktop を再起動してください。',
    ...
    native: {
      ...
      inputBody: 'Orin が応答を待っています。',
      turnDoneTitle: 'Orin が完了しました',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を Orin のデフォルトに戻しますか？',
    ...
    vault: {
      ...
      otpHint: '2FA を有効にするときにサイトが表示する「セットアップキー」。保存すると Orin がコードを生成します。',
      ...
      sources: {
        ...
        notInstalled: name =>
          `未検出です。${name} のコマンドラインツールをインストールしてサインインすると、Orin が自動的に検出します。`,
        disabledDesc: '検出済みですが、Orin では無効になっています。',
        lockedDesc:
          '検出済み。エージェントがログイン情報を必要とするときにロック解除を求めます。今すぐ解除することもできます。',
        unlockedDesc: 'このセッションでロック解除済み。30分間操作がないか Orin を閉じると自動的にロックされます。',
        statusLocked: 'ロック中',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Orin がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Orin が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Orin がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'Orin がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'Orin',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、Orin をシステム設定に合わせます。',
      ...
      reactionsDesc:
        'iMessage風の絵文字タップバック — メッセージにリアクションでき、Orinもあなたのメッセージにリアクションします。',
      tipsTitle: 'アプリ内ヒント',
      tipsDesc:
        'アプリや Orin からのヒントをときどき表示します。各ヒントは一度だけ表示されます。利用開始から30日後に自動でオフになりますが、再びオンにできます。',
      tipsReset: (count: number) => `${count}件のヒントをもう一度表示`,
      toursTitle: 'ガイドツアー',
      toursDesc:
        '各ステップを強調しながら、Orin がアプリを案内します。利用開始から30日後に自動でオフになりますが、再びオンにできます。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Orin の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Orin を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Orin がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Orin が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Orin が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Orin がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Orin 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Orin ランタイムは更新されましたが、デスクトップアプリ自体は古いビルドのままです。アプリを更新するまで、新しいインターフェース機能(Bot Mode など)は表示されません。下の更新を実行してアプリを再ビルドしてください。それでもこの警告が消えない場合は、最新のデスクトップインストーラーから再インストールしてください。',
      bundleOutOfSyncAction: 'インストーラーを入手',
      bundleSwapPending: '再起動して更新を完了',
      bundleSwapPendingDesc:
        '更新されたアプリはすでにインストール済みです。Orin を再起動するだけで新しいビルドが読み込まれます。チャットや設定はそのまま保持されます。',
      bundleSwapPendingAction: 'Orin を再起動',
      updates: '更新',
      checkNow: '今すぐ確認',
      checking: '確認中…',
      seeWhatsNew: '新機能を見る',
      ...
      automaticUpdatesDesc: 'Orin はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    config: {
      ...
      minimizeToTrayDesc:
        'ウィンドウの最小化やメインウィンドウを閉じる操作でシステムトレイ（macOS ではメニューバー）に隠し、Orin を実行し続けます。終了するにはトレイメニューの「Orin を終了」または Cmd+Q を使います。初期設定はオフで、このデバイスにのみ適用されます。',
    ...
    hudModifier: {
      ...
      permission:
        'システム設定 → プライバシーとセキュリティ → 入力監視で Orin を許可し、再試行してください。このジェスチャーはキー入力の記録や画面の撮影を行いません。',
      unavailable:
        'HUD ジェスチャーヘルパーを起動できなかったか、予期せず停止しました。再試行するか Orin を再起動してください。Orin 内の既存の HUD ショートカットは引き続き使用できます。',
      missingHelper:
        'この Orin には HUD ジェスチャーヘルパーが含まれていません。Orin を更新または再インストールしてから再試行してください。',
      unsupportedSession:
    ...
    screenshot: {
      ...
      enabledDesc:
        'どのアプリからでも左右の Command キーを同時に押すと、最前面のウインドウを撮影し、Orin の現在の下書きに添付します。自動送信はしません。初期設定はオフで、この Mac にのみ適用されます。機密情報が写る可能性があるため、送信前に添付画像を確認してください。',
      ...
      inputPermission:
        '入力監視の許可により、他のアプリがアクティブな間も両方の Command キーを検出できます。システム設定 → プライバシーとセキュリティ → 入力監視で Orin を許可し、ここに戻って再試行してください。',
      screenPermission:
        '画面収録の許可により、このショートカットを使ったときに最前面のアプリのウインドウを撮影できます。システム設定 → プライバシーとセキュリティ → 画面収録で Orin を許可し、ここに戻って再試行してください。macOS に求められた場合は Orin を再起動してください。',
    ...
    quickEntry: {
      enabledTitle: 'クイック入力',
      enabledDesc:
        'グローバルショートカットで小さな入力欄をどこからでも呼び出し、Orin を開かずにプロンプトを送信します。',
    ...
    gateway: {
      ...
      intro:
        'Orin Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Orin バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      ...
      localDesc:
        'ローカルホストでプライベートな Orin バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの Orin バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      ...
      restartingMessage: 'Orin Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · Orin ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'Orin は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— Orin を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。Orin は非対話的に ssh を実行します。',
      sshErrHostKey:
        '前回の接続以降、ホスト鍵が変更されています。想定どおりか確認し、ssh-keygen -R <host> を実行してから再接続してください。',
      sshErrNotInstalled:
        'リモートホストに Orin がインストールされていません。リモートでインストールする（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）か、Orin パスを設定してください。',
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。Orin Desktop の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
      sshErrTimeout: 'SSH 接続がタイムアウトしました。ホストが到達不能、またはスリープ中の可能性があります。',
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの Orin を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを Orin に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) =>
        `ビルド ${tag}、${backend} バックエンド。サーバーは Orin が起動・管理します。`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。Orin がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          ...
          body: 'Orin が既定ブラウザプロファイルのスナップショットでブラウジングできるようにすると、サイトはログイン済みの状態で開きます。',
  ...
  starmap: {
    ...
    emptyDesc: 'Orin がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Orin を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Orin を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Orin が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Orin デスクトップを設定' },
    ...
    hermesActiveSessions: (version, count) => `Orin ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: 'どちらの方法でも、あなたが管理するボットを接続し、資格情報はこの Orin にのみ保存されます。',
      quickSetup: 'クイックセットアップ',
      recommended: '推奨',
      quickHelp:
        'QR コードをスキャンして Telegram で確認します。Orin がボットを作成し、あなたの Telegram ユーザー ID を自動検出します。',
      createWithQr: 'QR で作成',
      starting: '開始中…',
      replaceWarning:
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: 'このプロファイルのセッションは、このパソコンではなく指定したリモートの Orin で実行されます。',
      urlLabel: 'リモートアドレス',
    ...
    defaultDescription: 'Orin の起動時と新しいチャットに使用します。既存のセッションのプロファイルは変わりません。',
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Orin 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  modelAssignment: {
    saveFailed: 'Orin はモデルの変更を保存しませんでした。',
  ...
  cron: {
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Orin が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには Orin バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'Orin から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには Orin バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    ...
    placeholderStarting: 'Orin を起動中...',
    placeholderReconnecting: 'Orin に再接続中…',
    ...
    attachUrlDesc: 'Orin がページを取得し、このターンのコンテキストとして含めます。',
  ...
  updates: {
    ...
    unsupportedMessage: 'このバージョンの Orin はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの Orin をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の Orin バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'Orin をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'Orin は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために Orin デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
      'Orin アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に Orin を再度開きます。更新中はご自分で Orin を開き直さないでください。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Orin が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 Orin が自動的に再度開きます。',
    errorTitle: '更新が完了しませんでした',
    errorBody: 'ご安心ください。何も失われていません。今すぐ再試行できます。',
    blockerTitle: 'Orin を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて Orin を更新',
    foreignBlockerBody:
      'Orin はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'Orin は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  guidedGreeting: {
    line: 'やあ、どうぞ。Orin です。二分だけください、あなたに合わせて整えます。それから、本当にやりたいことに取りかかりましょう。\n\nまずは、何とお呼びすればいいですか。',
  ...
  install: {
    ...
    oneTimeTitle: 'Orin には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'Orin Desktop をセットアップ',
    setupChoiceDesc:
      'すでに実行している Orin ゲートウェイに接続するか、このコンピューターに Orin をローカルインストールします。',
    connectExistingTitle: '既存の Orin に接続',
    connectExistingShort: '既存環境に接続',
    connectExistingDesc:
      'セッショントークンまたはブラウザーサインインでリモートバックエンドを使用します。ローカルインストールは開始されません。',
    installLocalTitle: 'Orin をローカルにインストール',
    installLocalDesc: 'Orin をダウンロードし、Python 環境を作成して、このコンピューターでバックエンドを実行します。',
    localStartUnavailable:
      'ローカルインストールを開始できません。Orin Desktop を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の Orin に接続',
    remoteSetupDesc:
      'ゲートウェイ URL を入力してください。Orin Desktop がトークンとブラウザーサインインのどちらが必要かを検出します。',
    remoteUrlTitle: 'ゲートウェイ URL',
    remoteUrlDesc: 'Orin ゲートウェイのベース URL を使用します。リモートの場合は https:// を含めてください。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: 'ゲートウェイ認証方式を検出中...',
    probeError: 'その Orin ゲートウェイに到達できませんでした。',
    identityProvider: 'ID プロバイダー',
    authTitle: '認証',
    ...
    settingUpTitle: 'Orin Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Orin CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Orin インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Orin インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'Orin Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'Orin はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'Orin を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — Orin を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Orin を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — Orin が自動接続します',
    ...
    authorizeThere: 'そこで Orin を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。Orin をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Orin は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Orin を再起動中...',
      askRestart: 'Orin にサーバーの再起動を依頼',
      lookingRestart: taskId => `Orin は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Orin はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、Orin にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `Orin がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Orin がサーバーを再起動できませんでした。',
      stillWorking:
        'Orin はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Orin が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'Orin ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.hermes/config.yaml) に追加されます。Orin はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    secretDesc: 'Orin は続行するための認証情報が必要です。',
    ...
    vaultSaveDesc: origin =>
      `Orin は ${origin} のサインインページに到達しましたが、保存されたログイン情報がありません。ここで一度入力すると、このマシン上で暗号化して保存され、ページに直接入力されます。モデルはパスワードを一切見ません。`,
    ...
    vaultCodeDesc: site =>
      `${site} がワンタイムコード（SMS、メール、または認証アプリ）を求めています。ここに入力すると Orin がページに入力します。モデルはコードを一切見ません。`,
    vaultCodeLabel: 'コード',
    vaultCodeFootnote:
      'ヒント：「設定 → パスワードとログイン」でこのログインに認証キーを保存すると、Orin がコードを自動入力します。',
    vaultCodeSkip: 'スキップ',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Orin Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには Orin Desktop を再起動してください',
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '一度教えれば覚えます',
        text: 'スキルは手順書のフォルダで、必要な場面で Orin が自分で読み込みます。'
      },
      messaging: {
        title: 'デスクを離れても Orin',
      ...
      profiles: {
        ...
        text: 'それぞれが独自のキー・メモリ・セッションを持つ、別の Orin です。'
      },
      'composer-mentions': {
        title: 'ファイルとコマンド',
