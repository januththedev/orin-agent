...
describe('speaker labels', () => {
  it('relabels Orin control-frame openers only in member-authored transcript lines', async () => {
  ...
  it('the default profile speaks as Orin in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Orin: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale Orin', async () => {
    ...
    // "Orin is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → Orin, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('Orin')
  ...
  it('qualifies twins per room and never renders a raw key when the roster row is missing', async () => {
    ...
    expect(chat.groupSpeakerLabel('local::reviewer')).toBe('reviewer')
    expect(chat.groupSpeakerLabel('spark::default')).toBe('Orin')
