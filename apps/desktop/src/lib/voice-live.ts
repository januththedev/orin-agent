...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Orin.
 *
 ...
 * that into an ordinary Orin turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Orin keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Orin) for help. `context` is the recent
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Orin; late results for an
  ...
  /** Quiet progress for the live model ("Orin is running the tests…"). */
