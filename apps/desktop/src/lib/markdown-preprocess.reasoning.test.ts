...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> Orin Desktop')).toBe('no Orin Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>Orin')).toBe('no Orin')
  })
