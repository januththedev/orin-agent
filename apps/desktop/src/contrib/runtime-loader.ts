...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update Orin Desktop to load larger plugins"
    )
  }
...
export async function uninstallDiskPlugin(pluginId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  if (!remove) {
    return { ok: false, error: 'this Orin Desktop build cannot remove desktop plugins — delete the folder by hand' }
  }
