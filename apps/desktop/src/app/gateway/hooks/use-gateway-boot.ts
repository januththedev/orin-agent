...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting Orin...". Once the
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Orin…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Orin backend'
        )
        ...
        // as an opaque "Could not connect to Orin gateway". resolveGatewayWsUrl
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Orin Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen({ explicit: true }))) {
          throw new Error('Orin gateway is not connected')
        }
    ...
    async function boot() {
      ...
      try {
        ...
        // round-trip must not hang "Starting Orin…" forever. Initial boot
        ...
        const conn = await withTimeout(
          ...
          BACKEND_BOOT_WAIT_TIMEOUT_MS,
          'Timed out connecting to Orin backend'
        )
        ...
        // reaches the recovery affordance instead of hanging "Starting Orin…".
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "Orin couldn't start"
