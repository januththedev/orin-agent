...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  ...
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForHermes) → app sits in the connecting combo, then fails', async () => {
    ...
    await act(async () => {
      rejectConn(new Error('Orin backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // "Starting Orin…" forever; the renderer needs to own its own bound
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote Orin gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Orin couldn't start" overlay — the socket is open
  ...
  it('a backend exit while the boot overlay is up fails the overlay and does not add a dead-button toast', async () => {
    ...
    // Orin" toast here would do nothing when clicked; the overlay's own
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Orin')
    expect($currentCwd.get()).toBe('C:\\Orin')
  })
  ...
  it('a failed cold boot keeps its recovery surface while main replays cold-boot progress behind it (#112899)', async () => {
    ...
    const desktop = fakeDesktop()
    desktop.getConnection = vi.fn(async () => {
      throw new Error('Orin backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example')
    })
    desktop.getBootProgress = vi.fn(async () => ({
      error: 'Orin backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example',
      fakeMode: false,
      message: 'Desktop boot failed',
      phase: 'backend.error',
      progress: 24,
    ...
    for (let replay = 0; replay < 2; replay += 1) {
      act(() => {
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Resolving Orin backend',
        ...
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Connecting to remote Orin backend at https://gateway.tailnet.example:8443',
          phase: 'backend.remote',
          progress: 24,
          running: true,
          timestamp: Date.now()
