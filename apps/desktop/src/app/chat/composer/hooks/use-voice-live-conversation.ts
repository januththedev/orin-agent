...
interface VoiceLiveConversationOptions {
  ...
  enabled: boolean
  onFatalError?: () => void
  /** Interrupt the in-flight Orin turn (Stop-button seam). Fired when a new
   *  delegation supersedes one still running. */
  onInterrupt?: () => Promise<void> | void
  onStopWord?: () => void
  /** Submit a Orin turn: `text` is the user's last words (the bubble and the
...
/** Turn transcript fragments into the Orin turn: `prompt` is what the user
 ...
 * Orin. There is no `transcribing` phase: the voice model owns speech.
 */
export function useVoiceLiveConversation({
    ...
    const session = new VoiceLiveSession(
      {
        ...
        onDelegation: (delegationId, context) => {
          ...
          void Promise.resolve(latest.current.onSubmit(prompt, voiceContext)).catch(error => {
            ...
            session.speak(delegationId, 'Sorry, I could not reach Orin for that request.')
            setDelegation(null)
            refreshStatus()
          })
        },
  ...
  // Drive the reply back into the voice: stream commentary as Orin writes
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (tool && tool !== lastToolLabelRef.current) {
        ...
        session.think(delegationId, `Orin is working: ${tool}. Not done yet.`)
      }
      ...
      if (
        ...
        if (spokenLengthRef.current === 0) {
          session.think(delegationId, 'Orin finished that request without a spoken result.')
        }
