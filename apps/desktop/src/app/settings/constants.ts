...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Orin & Nous-trained models',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // gpt-live branch (one full-duplex voice model delegating to Orin).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  browser: {
    useRealProfile:
      "Local browsing uses your real logins. Orin copies your default browser's profile (cookies, logins, preferences) into a managed snapshot and drives it with its packaged Chromium — your live profile is never opened directly, and the copy is refreshed from it on each run. Also lets the agent open a local real-profile session on request even when a cloud browser backend is configured. Only Chromium browsers (Chrome, Edge, Brave, Brave Origin, Chromium) are supported; a non-Chromium default fails with a clear message. Off by default."
  },
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Orin stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Orin can read from one file request.',
  approvals: {
    mode: 'How Orin handles commands that need explicit approval.',
  ...
  voice: {
    ...
    voiceChatMode:
      'chained: speech-to-text → Orin → text-to-speech with the providers below. gpt-live: one full-duplex OpenAI voice model (gpt-live-1) listens and talks, and hands every real request to Orin — any model you have selected answers with the full toolset. Needs an OpenAI API key; the voice layer bills $0.05 per minute.',
    gptLive: {
      voice: 'Voice for GPT-Live mode. Custom voice IDs are accepted.',
      instructions:
        'Extra sentences for the live voice persona (tone, pace, language). Orin keeps its own system prompt.'
    }
  },
  tts: {
    xai: {
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Orin updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
