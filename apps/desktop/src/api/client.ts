...
// "Timed out connecting to Orin backend" that hangs the UI (#48504).
//
...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Orin gateway connection closed',
      connectErrorMessage: 'Could not connect to Orin gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Orin gateway is not connected',
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] Orin Desktop has no server-request registry for ${request.method} (${request.id})`),
