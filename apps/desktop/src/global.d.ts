...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      // Orin Cloud: one portal login powers discovery + silent per-agent
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Orin Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Orin Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Orin Cloud org (slug or id) to
...
// A discovered Orin Cloud agent — the trimmed DTO from NAS GET /api/agents.
