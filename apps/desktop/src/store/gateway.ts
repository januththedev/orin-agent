...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Orin gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.hermesDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Orin Desktop.')
  }
...
export async function ensureActiveGatewayOpen({
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Orin gateway is not connected" on the Sessions `+` action (#88880).
